...
#include "../../../include/tool/toolSettings/effects/TSet_FX_Fill.h"
...
TSet_FX_Fill::TSet_FX_Fill()
{
	...
	this->colorUsage = TSetProp::background; // 8100
...
TSetControl_Node* TSet_FX_Fill::getControlNode(int settingSig, int subSig)
{
	return nullptr;
}
TSetController* TSet_FX_Fill::getController(int settingSig, int subSig)
{
	return nullptr;
}
TSetGraph* TSet_FX_Fill::getGraph(int settingSig, int subSig)
{
	return nullptr;
}
TSetNoise* TSet_FX_Fill::getNoise(int settingSig, int subSig)
{
	return nullptr;
}
...
void TSet_FX_Fill::initializeData(CColor FGColor, CColor BGColor, int tipSize)
{
	...
	switch (colorUsage)
	{
	case TSetProp::foreground: outFinalColor = RGB_toHSL(glm::dvec3((double)FGColor.r, (double)FGColor.g, (double)FGColor.b)); break;
	case TSetProp::background: outFinalColor = RGB_toHSL(glm::dvec3((double)BGColor.r, (double)BGColor.g, (double)BGColor.b)); break;
	...
	*/
	...
	finalColor.a = lerpf(mixAmountMin, mixAmountMax, (float)distr(eng));
	...
	if (maskUsingAngle) { maskAngle = lerpf(maskAngleMin, maskAngleMax, (float)distr(eng)); }

	// Determine XY Mask Center Offset
	if (maskUsingCenter)
	{
		maskCenter_offsetX = lerpf(maskCenter_offsetXMin, maskCenter_offsetXMax, (float)distr(eng));
		maskCenter_offsetY = lerpf(maskCenter_offsetYMin, maskCenter_offsetYMax, (float)distr(eng));
