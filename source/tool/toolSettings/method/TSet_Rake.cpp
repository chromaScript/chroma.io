...
#include "../../../include/tool/toolSettings/method/TSet_Rake.h"
	...
	int settingsSig, int subSig, std::shared_ptr<CObject> object, bool isGet, bool asPercentage, bool asString)
{
	...
	else if (settingsSig >= 5630 && settingsSig < 5650)
	{
		...
		else if (settingsSig == 5632)
		{
			if (isGet)
			{
				if (asString) { return std::make_shared<CObject>(makeSettingString(pointsCount, settingsSig, subSig, "pointsCount")); }
				if (asPercentage) {
					return std::make_shared<CObject>(double(
						percentRange((float)pointsCount, TSET_RAKE_POINTS_MIN, TSET_RAKE_POINTS_MAX, true)));
				}
			...
			else
			{
				...
				else { pointsCount = clampi((int)std::round((int)std::get<double>(object.get()->obj)), TSET_RAKE_POINTS_MIN, TSET_RAKE_POINTS_MAX); }
			}
		}
...
TSetControl_Node* TSet_Rake::getControlNode(int settingSig, int subSig)
{
	return nullptr;
}
TSetController* TSet_Rake::getController(int settingSig, int subSig)
{
	return nullptr;
}
TSetGraph* TSet_Rake::getGraph(int settingSig, int subSig)
{
	return nullptr;
}
TSetNoise* TSet_Rake::getNoise(int settingSig, int subSig)
{
	return nullptr;
}