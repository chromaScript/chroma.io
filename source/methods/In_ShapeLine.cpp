#include "../include/methods/In_ShapeLine.h"
...
int In_ShapeLine::move(Application* sender, MouseEvent dat)
{
		...
		if (controlScheme == TSetType::drag)
		{
			...
			for (int i = 0; i < verts.size(); i++)
			{
				...
				if (drag.pressureMapping)
				{
					...
					{
						fragData.anchors[size_t(i)].pressure -= drag.pressureMapDecayRate / ((moveMode || rotateMode) ? 1000.0f : 100.0f);
...
int In_ShapeLine::click(Application* sender, MouseEvent dat)
{
