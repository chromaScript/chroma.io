...
#define DT_STROKE_SPLINEBRUSH 72
...
public:
	...
	std::shared_ptr<CustomCursor> getCursor(CursorType name);
	// Check Functions
	bool checkValidIOCombination(std::shared_ptr<CInterpreter> interpreter, std::string inputName, std::string outputName);
	bool checkValidIOCombination(std::shared_ptr<CInterpreter> interpreter, MethodType inputMacro, MethodType outputMacro);
	bool checkValidControlScheme(std::shared_ptr<CInterpreter> interpreter, std::string inputName, std::string controlScheme);
	bool checkValidControlScheme(std::shared_ptr<CInterpreter> interpreter, MethodType inputMacro, MethodType controlMacro);
	MethodType convertIOStringToMacro(int kind, std::string name);
	TSetType convertControlMacro(MethodType macro);
	// Input Functions
	...
	int checkToolHotkeys(Keybind keybind);
	std::shared_ptr<Tool> getActiveTool();
	bool checkActiveTool();
