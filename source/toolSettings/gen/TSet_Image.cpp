...
#include "../../include/toolSettings/gen/TSet_Image.h"
	...
	int settingsSig, int subSig, std::shared_ptr<CObject> object, bool isGet, bool asPercentage, bool asString)
{
	...
	else if (settingsSig == 2201)
	{
		if (isGet)
		{
			if (asString) { return std::make_shared<CObject>(makeSettingString(tipSize, settingsSig, subSig, "tipSize", false)); }
			if (asPercentage) {
				return std::make_shared<CObject>(double(
					percentRange_cubed(tipSize, TSET_IMAGE_TIPSIZE_MIN, TSET_IMAGE_TIPSIZE_MAX, true)));
			}
		...
		else
		{
			if (asPercentage) {
				tipSize = (int)std::round(
					percentRange_cubed(std::get<double>(object.get()->obj), TSET_IMAGE_TIPSIZE_MIN, TSET_IMAGE_TIPSIZE_MAX, false));
			}
			else {
				tipSize = clampi(
